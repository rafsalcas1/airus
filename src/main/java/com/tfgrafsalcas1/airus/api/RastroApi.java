...
import com.tfgrafsalcas1.airus.deserialize.OpenSkyRastroDeserializer;
...
public class RastroApi {
	 ...
	 */
	public RastroApi(String username, String password) {
		// set up JSON mapper
		mapper = new ObjectMapper();
		SimpleModule sm = new SimpleModule();
		sm.addDeserializer(Rastro.class, new OpenSkyRastroDeserializer());
	 ...
	 */
    private Rastro getResponse(String baseUri) throws IOException {
        ...
        if (!response.isSuccessful()) {
			throw new IOException("Could not get OpenSky Plane track, response " + response);
		}
        ...
        if (charset != null) {
            return mapper.readValue(new InputStreamReader(response.body().byteStream(), charset), Rastro.class);
        } else {
	 ...
	 * @throws IOException if there was an HTTP error
	 */
	private Rastro getOpenSkyRastro(String baseUri) throws IOException {
	 ...
	 */
	public Rastro getRastro(int time, String icao24) throws IOException {
		return getOpenSkyRastro(API_ROOT + "/tracks/all?icao24="+icao24+"&time="+time);
	}
}
