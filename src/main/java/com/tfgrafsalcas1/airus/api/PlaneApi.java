...
import com.tfgrafsalcas1.airus.deserialize.OpenSkyPlaneDeserializer;
...
public class PlaneApi {
	 ...
	 */
	public PlaneApi(String username, String password) {
		// set up JSON mapper
		mapper = new ObjectMapper();
		SimpleModule sm = new SimpleModule();
		sm.addDeserializer(Plane.class, new OpenSkyPlaneDeserializer());
	 ...
	 */
    private Plane getResponse(String baseUri) throws IOException {
        ...
        if (charset != null) {
            return mapper.readValue(new InputStreamReader(response.body().byteStream(), charset), Plane.class);
        } else {
	 ...
	 * @throws IOException if there was an HTTP error
	 */
	private Plane getOpenSkyPlane(String baseUri) throws IOException {
	 ...
	 */
	public Plane getPlane(int time, String icao24) throws IOException {
		return getOpenSkyPlane(API_ROOT + "/tracks/all?icao24="+icao24+"&time="+time);
	}
}
